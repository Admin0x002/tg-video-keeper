...
**一句话目标：** 私密 Telegram UserBot，监控本人收藏夹（Saved Messages），将手动转发进来的
视频/图片/媒体消息**克隆**（非转发）到私密备份频道，防止原频道被封导致媒体失效。
仅处理本人收藏夹，**不对外提供任何公开交互服务**。
...
| Telegram 库 | **Telethon** | 1.44.0（v1 稳定分支） | 主流 MTProto UserBot 库；`events.Album` 原生处理相册；`send_file(file=msg)` 干净克隆媒体无转发头 |
...
3. `send_file(entity, file=message)` 克隆语义清晰——把消息对象当 file 传入，
...
# 安装依赖（uv 自动创建 .venv + uv.lock）
uv sync

# 复制配置模板并填写
cp .env.example .env
# 编辑 .env 填入 API_ID / API_HASH / TARGET_CHAT_ID
...
# 应用滚动日志
tail -f /opt/tg-video-keeper/logs/keeper.log
```
...
2. **禁止使用 `forward_messages` / `message.forward_to`。** 必须用 `send_file(file=...)`
...
6. **仅监听收藏夹。** 事件处理器硬编码 `chats=["me"]`，仅处理来自收藏夹的消息。
...
用户手机浏览其他频道
  └─ 遇到喜欢的视频 → 手动转发到 [收藏夹]
       └─ keeper.py 监听到新消息
            ├─ 有媒体？无媒体则忽略（纯文本不克隆）
            ├─ 单条媒体 → send_file(file=msg) 克隆到 TARGET
            └─ 相册（grouped_id）→ 等 Album 事件聚合 → 作为相册整体克隆
       └─ 克隆成功后
            └─ DELETE_ORIGINAL_FROM_SAVED=true → 删除收藏夹原消息
```

## 7. 安全模型

- **仅监听收藏夹**（`chats=["me"]`），只处理本人消息。
- 收藏夹天然安全：只有账号本人能向收藏夹发消息。
